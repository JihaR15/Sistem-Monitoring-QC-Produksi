...
export const StatCard = ({ title, value, color = "text-neutral-900 dark:text-white", icon }) => (
    <div className="bg-white dark:bg-neutral-900 p-5 rounded-xl border border-neutral-100 dark:border-neutral-800 shadow-md flex items-center gap-4 hover:border-blue-400 dark:hover:border-blue-600 transition-all hover:shadow-lg group">
...
export const SelectSimple = ({ value, onChange, options, placeholder }) => (
    <div className="relative">
        <select className="appearance-none w-full p-2.5 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700 rounded-lg text-sm outline-none focus:border-blue-500 transition-colors dark:text-white"
            ...
            <option value="" className="dark:bg-neutral-900">- {placeholder} -</option>
            {options.map(o => <option key={o} value={o} className="dark:bg-neutral-900">{o}</option>)}
        </select>
